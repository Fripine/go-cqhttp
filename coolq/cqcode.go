...
import (
	...
	"github.com/Fripine/go-cqhttp/internal/mime"
	"github.com/Fripine/go-cqhttp/internal/msg"
	"github.com/Fripine/go-cqhttp/internal/param"
	"github.com/Fripine/go-cqhttp/pkg/onebot"
)
...
func ToMessageContent(e []message.IMessageElement, source message.Source) (r []global.MSG) {
	for _, elem := range e {
		...
		case *message.GroupImageElement:
			data := global.MSG{"file": o.Name, "url": o.Url, "subType": uint32(o.ImageBizType)}
			switch {
			case o.Flash:
				data["type"] = "flash"
			case o.EffectID != 0:
