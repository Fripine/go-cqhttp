...
func lastVersion() (string, error) {
	r, err := download.Request{URL: "https://api.github.com/repos/Fripine/go-cqhttp/releases/latest"}.JSON()
	if err != nil {
		return "", err
	}
...
func CheckUpdate() {
	...
	if global.VersionNameCompare(base.Version, latest) {
		logrus.Infof("当前有更新的 go-cqhttp 可供更新, 请前往 https://github.com/Fripine/go-cqhttp/releases 下载.")
...
func checksum(github, version string) []byte {
	sumURL := fmt.Sprintf("%v/Fripine/go-cqhttp/releases/download/%v/go-cqhttp_checksums.txt", github, version)
...
func SelfUpdate(github string) {
	...
	url := fmt.Sprintf("%v/Fripine/go-cqhttp/releases/download/%v/%v", github, latest, binaryName())
